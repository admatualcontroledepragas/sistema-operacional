// ==========================================
        ...
        function prepararFormularioNovo(tipo) {
            fecharModal('modal-selecao');
            idEmEdicao = null;
            
            document.getElementById('form-dedetizacao').style.display = 'none';
            document.getElementById('form-limpeza').style.display = 'none';
            document.getElementById('form-termo').style.display = 'none';
            ...
            if (tipo === 'Dedetizacao') {
                document.getElementById('form-dedetizacao').style.display = 'block';
                document.getElementById('titulo-formulario').innerText = 'Novo Comunicado - Dedetização';
                document.getElementById('btn-salvar-dedetizacao').innerText = 'Salvar Comunicado';
                ativarCard(1);
            } else if (tipo === 'Limpeza') {
                document.getElementById('form-limpeza').style.display = 'block';
                document.getElementById('titulo-formulario').innerText = 'Novo Comunicado - Limpeza';
                document.getElementById('btn-salvar-limpeza').innerText = 'Salvar Comunicado';
            } else if (tipo === 'Termo') {
                document.getElementById('form-termo').style.display = 'block';
                document.getElementById('titulo-formulario').innerText = 'Novo Comunicado - Fumacê';
        ...
        async function aplicarFiltroPrincipal() { 
            ...
            if (res.sucesso) { 
                ...
                if (dashTotal) dashTotal.innerText = res.total || res.registros.length;
        ...
        function setFiltroPrincipal(filtro, btnElement) {
            ...
            if (lblTotal) lblTotal.innerText = "Total de " + tituloMetrica;
        ...
        async function aplicarFiltrosEBusca() {
            ...
            const res = await fetchAPI('listar', { 
                aba: filtroAtualHistorico, limite: itemsPerPage, pagina: currentPage,
                busca: searchCliente, dataInicio: dataInicioStr, dataFim: dataFimStr
            }, "Buscando histórico...");
        ...
        async function aplicarEngenhariaReversa(id, tipo, isImportacao = false) {
            ...
            if (tipo === 'Dedetização' || tipo === 'Dedetizacao') {
                ...
                if(!isImportacao) {
                    idEmEdicao = id;
                    document.getElementById('titulo-formulario').innerText = 'Editando Comunicado - Dedetização';
                ...
                function preencherPromoReversa(idCard, textoStr) {
                    ...
                    memoriaPragas[idCard] = []; memoriaPragasOutrosTxt[idCard] = "";
                ...
                if(!isImportacao) mostrarView('view-formulario');
                ativarCard(1);
                ...
                if(!isImportacao) {
                    idEmEdicao = id;
                    document.getElementById('titulo-formulario').innerText = 'Editando Comunicado - Limpeza';
                ...
                if(!isImportacao) {
                    idEmEdicao = id;
                    document.getElementById('titulo-formulario').innerText = 'Editando Comunicado - Fumacê';
        ...
        async function editarRegistro(id, tipo) {
            fecharFormulario(); // Fecha qualquer visão prévia
            await aplicarEngenhariaReversa(id, tipo, false);
        }

        function abrirLinkPdf(link) {
            if (!link || link === "undefined" || link.includes("Erro") || link === "") { showToast("PDF indisponível.", "error"); return; }
        ...
        async function baixarPDF(id, tipo) {
            try {
                ...
                const res = await fetchAPI('iniciarCompartilhamentoPDF', { id: id, tipo: tipo }, "Baixando PDF...");
                if (!res.sucesso) { showToast(res.mensagem, "error"); return; }
                const base64Data = res.base64; const contentType = 'application/pdf';
                const byteCharacters = atob(base64Data); const byteArrays = [];
                ...
                const blob = new Blob(byteArrays, { type: contentType }); const nomeDoArquivo = res.nomeArquivo || "Documento.pdf";
                const url = window.URL.createObjectURL(blob); const a = document.createElement('a');
                a.href = url; a.download = nomeDoArquivo; document.body.appendChild(a); a.click();
        ...
        function desenharLinhas(registros, tbodyId) {
            ...
            registros.forEach(item => {
                ...
                let botoesAcao = `
                    <button class="icon-btn" onclick="abrirLinkPdf('${item.linkPdf}')" title="Abrir"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg></button>
                    <button class="icon-btn share" onclick="baixarPDF('${item.id}', '${item.servico}')" title="Baixar PDF"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg></button>
                    <button class="icon-btn edit" onclick="editarRegistro('${codigoSeguro}', '${item.servico}')" title="Editar"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button>
                    <button class="icon-btn delete" onclick="abrirModalExclusao('${codigoSeguro}')" title="Excluir"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
                `;
                tr.innerHTML = `
                    <td style="max-width: 250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${item.cliente || '-'}"><strong>${item.cliente || '-'}</strong></td>
                    <td style="white-space: nowrap;">${item.servico || '-'}</td>
                    <td style="white-space: nowrap;">${item.data || '-'}</td>
                    <td class="actions" style="white-space: nowrap; width: 140px;">${botoesAcao}</td>
                `;
        ...
        function inserirModeloLimpeza() {
            ...
            if (editor.innerHTML.trim() !== '' && editor.innerHTML.trim() !== '<br>') { if(!confirm("Atenção: A caixa de texto já possui conteúdo. Deseja substituí-lo pelo modelo padrão?")) return; }
        ...
        [1, 2, 3].forEach(id => {
            document.getElementById(`imovel-${id}`).addEventListener('change', function() {
                ...
                if (this.value === "Outro") { txtInput.classList.remove('hidden'); txtInput.focus(); } else { txtInput.classList.add('hidden'); txtInput.value = ''; }
            });
            document.getElementById(`serv-outro-cb-${id}`).addEventListener('change', function() {
                ...
                if (this.checked) { txtInput.classList.remove('hidden'); txtInput.focus(); } else { txtInput.classList.add('hidden'); txtInput.value = ''; }
            });
        });
        ...
        document.querySelectorAll('.servico-cb-1, .servico-cb-2, .servico-cb-3').forEach(cb => {
            cb.addEventListener('change', function() {
                ...
                const servico = this.value; const isChecked = this.checked; let pragasAfetadas = [];
        ...
        function ativarCard(idCard) {
            cardAtivo = idCard; document.querySelectorAll('.promo-card').forEach(card => card.classList.remove('active')); document.getElementById('card-' + idCard).classList.add('active');
            ...
            if (memoriaPragas[idCard].includes("OUTROS")) { txtOutrosPragas.classList.remove('hidden'); txtOutrosPragas.value = memoriaPragasOutrosTxt[idCard]; } else { txtOutrosPragas.classList.add('hidden'); txtOutrosPragas.value = ""; }
        }
        ...
        function atualizarVisorPragas(idCard) {
            const display = document.getElementById('display-pragas-' + idCard); let pragasParaMostrar = [...memoriaPragas[idCard]];
        ...
        function showToast(message, type = 'success') {
            ...
            toast.className = `toast ${type}`; toast.innerHTML = `<span>${message}</span>`; container.appendChild(toast);
        ...
        function toggleLoader(show, text = "Processando, aguarde...") {
            const loader = document.getElementById('global-loader'); const loaderText = document.getElementById('global-loader-text');
            if (show) { loaderText.innerText = text; loader.classList.remove('hidden'); } else { loader.classList.add('hidden'); }
        }

        async function fetchAPI(action, dados = {}, mensagemLoader = "Processando, aguarde...") {
            try {
                toggleLoader(true, mensagemLoader); const controller = new AbortController(); const timeoutId = setTimeout(() => controller.abort(), 45000);
                const response = await fetch(URL_SCRIPT, { method: 'POST', body: JSON.stringify({ action: action, dados: dados }), signal: controller.signal });
                clearTimeout(timeoutId); const result = await response.json(); toggleLoader(false); return result;
            } catch (error) { 
                toggleLoader(false); 
                if (error.name === 'AbortError') showToast("Servidor demorou muito para responder.", "error"); else showToast("Falha na conexão.", "error"); 
        ...
        document.getElementById('form-dedetizacao').addEventListener('submit', async (e) => {
            e.preventDefault(); const btnSalvar = document.getElementById('btn-salvar-dedetizacao'); btnSalvar.disabled = true; btnSalvar.textContent = "Processando...";
            const dados = {
                id: idEmEdicao, cliente: document.getElementById('cliente').value, data: document.getElementById('data').value, previsao: `${document.getElementById('hora-inicio').value} às ${document.getElementById('hora-fim').value}`,
            ...
            if (res.sucesso) { showToast(res.mensagem, 'success'); fecharFormulario(); aplicarFiltroPrincipal(); dispararBuscaHistorico(); } else { showToast("Erro: " + res.mensagem, 'error'); }
        ...
        document.getElementById('form-limpeza').addEventListener('submit', async (e) => {
            e.preventDefault(); const btnSalvar = document.getElementById('btn-salvar-limpeza'); btnSalvar.disabled = true; btnSalvar.textContent = "Processando...";
            const dados = { id: idEmEdicao, cliente: document.getElementById('cliente-limp').value, data: document.getElementById('data-limp').value, vendedor: document.querySelector('input[name="vendedor-limp"]:checked').value, observacao: document.getElementById('obs-limp').innerHTML };
        ...
        document.getElementById('form-termo').addEventListener('submit', async (e) => {
            e.preventDefault(); const btnSalvar = document.getElementById('btn-salvar-termo'); btnSalvar.disabled = true; btnSalvar.textContent = "Processando...";
            const dados = { id: idEmEdicao, cliente: document.getElementById('cliente-termo').value, data: document.getElementById('data-termo').value, previsao: `${document.getElementById('hora-inicio-termo').value} às ${document.getElementById('hora-fim-termo').value}`, vendedor: document.querySelector('input[name="vendedor-termo"]:checked').value };
        ...
        document.getElementById('btn-confirmar-exclusao').addEventListener('click', async () => {
            if (!idParaExcluir) return; fecharModal('modal-exclusao');
            const res = await fetchAPI('excluir', { id: idParaExcluir }, "Excluindo Comunicado e PDF...");
            if (res.sucesso) { showToast(res.mensagem, 'success'); aplicarFiltroPrincipal(); dispararBuscaHistorico(); } else { showToast("Erro ao excluir: " + res.mensagem, 'error'); }
        });
